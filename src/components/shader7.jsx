...
const ShaderPlane = ({
  ...
  useEffect(() => {
    ...
    if (fluidMaterial.current) {
      const resolution = fluidMaterial.current.uniforms.u_resolution.value;
      resolution.set(size.width, size.height);
    }

    if (displayMaterial.current) {
      const resolution = displayMaterial.current.uniforms.u_resolution.value;
  ...
  useFrame(({ gl, clock, camera }) => {
    ...
    if (fluidMaterial.current) {
      ...
      fluidMaterial.current.uniforms.u_mouse.value.set(mouse.current.x, mouse.current.y, mouse.current.prevX, mouse.current.prevY);
      fluidMaterial.current.uniforms.u_resolution.value.set(size.width, size.height);
    }
    ...
    if (displayMaterial.current) {
      ...
      displayMaterial.current.uniforms.u_resolution.value.set(size.width, size.height, 1);
      displayMaterial.current.uniforms.u_fluid.value =
        currentFluidTarget.current.texture;
    }
...
const Shader7 = ({
  ...
  return (
    <section className={className} color={color} style={{ pointerEvents: 'auto' }}>
